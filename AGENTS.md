...
IT Service Management monorepo. **Design and implementation phases are complete (Full Release — Tag `v1.0.0`).** `docs/` is dense and authoritative (~6.3k lines, Indonesian); the codebase has grown to 18 models, 24 migrations, 727 passing backend tests (3300+ assertions), full ticket lifecycle with authorization & workflows (Tag `v0.3.0`), background SLA breach scheduler (4a), notification API (4b), audit log API with scoping & timezone filters (Tag `v0.4.0`), asset management, knowledge base, file attachments, 4 dashboard APIs for employee, technician, manager, and admin (Tag `v0.6.0`), complete Frontend Foundation, App Shell, 16 baseline UI + 9 shared components, 352 passing frontend unit/component tests, 30s background-safe polling notifications, 33 app routes, role-based visual analytics dashboards with Recharts, WCAG 2.2 AA accessibility audit, automated Playwright E2E testing (Tag `v0.9.0`), multi-stage production Docker stack, GitHub Actions CI, Octane audit, memory-safe streamed CSV export, and complete Definition of Technical Success & DoD verification (Tag `v1.0.0`). Sudah termasuk halaman: ticket list, detail, create; asset management; knowledge base; admin users, master data, audit logs; profile; dashboard analytics untuk 4 role; export CSV.
