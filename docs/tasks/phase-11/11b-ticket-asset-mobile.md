...
- [x] **Step 1 — RED:** tulis test render `TicketCard` (nomor, badge, judul, SLA) dan navigasi klik.
- [x] **Step 2 — Jalankan test, pastikan gagal:**
  ```bash
  npm run test TicketCard --prefix apps/web
  ```
- [x] **Step 3 — GREEN:** implementasikan `TicketCard`, lalu pasang sebagai `renderCard` di `TicketTable`.
- [x] **Step 4 — Verifikasi:** `npm run test TicketCard TicketTable --prefix apps/web` lulus, `npm run lint --prefix apps/web` bersih.
- [x] **Step 5 — Commit:**
  ```bash
...
- [x] **Step 1 — RED:** test render aksi sesuai `available_actions` dan tombol komentar memicu callback.
- [x] **Step 2 — Jalankan test, pastikan gagal.**
- [x] **Step 3 — GREEN:** implementasikan komponen + integrasi ke `page-client.tsx` (hanya tampil `< 640px`).
- [x] **Step 4 — Verifikasi:** test lulus, cek manual di viewport 390px tidak ada konten tertutup.
- [x] **Step 5 — Commit:**
  ```bash
...
- [x] **Step 1 — RED:** test `AssetCard` + test atribut input file.
- [x] **Step 2 — Jalankan test, pastikan gagal.**
- [x] **Step 3 — GREEN:** implementasikan `AssetCard`, pasang sebagai `renderCard`, perbarui `file-upload.tsx`.
