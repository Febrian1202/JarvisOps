  ...
  return (
    <article className="space-y-6">
      <div className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-xs space-y-4">
        ...
        <h1 className="font-bold text-xl sm:text-2xl text-foreground tracking-tight break-words">{article.title}</h1>
        ...
        <div className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-xs">
          ...
          <ul className="space-y-2">
            {article.related_articles.map((ra) => (
              <li key={ra.id}>
                <Link href={`/knowledge/${ra.slug}`} className="text-sm text-primary hover:underline break-words">
                  {ra.title}
                </Link>
              </li>
            ))}
